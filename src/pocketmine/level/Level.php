...
public function useItemOn(Vector3 $vector, Item &$item, int $face, float $fx = 0.0, float $fy = 0.0, float $fz = 0.0, Player $player = null) : bool{
    ...
    if($hand->isSolid() === true and $hand->getBoundingBox() !== null){
        ...
        if($player !== null){
            ...
            $blockBB = $hand->getBoundingBox();
            

            $playerCenterY = $playerBB->minY + 0.9; 
            $playerCenterX = ($playerBB->minX + $playerBB->maxX) / 2;
            $playerCenterZ = ($playerBB->minZ + $playerBB->maxZ) / 2;            

            $margin = 0.2;
            
            if($playerCenterX >= ($blockBB->minX - $margin) && 
               $playerCenterX <= ($blockBB->maxX + $margin) &&
               $playerCenterY >= ($blockBB->minY - $margin) && 
               $playerCenterY <= ($blockBB->maxY + $margin) &&
               $playerCenterZ >= ($blockBB->minZ - $margin) && 
               $playerCenterZ <= ($blockBB->maxZ + $margin)){
                ++$realCount;
            }
        }
