		...
		if($command === ""){
			...
			if(isset($commands[$pageNumber - 1])){
				foreach($commands[$pageNumber - 1] as $command){
					$sender->sendMessage(TextFormat::LIGHT_PURPLE . "/" . $command->getName() . ": " . TextFormat::GRAY . $command->getDescription());
				}
			}
			...
			if(($cmd = $sender->getServer()->getCommandMap()->getCommand(strtolower($command))) instanceof Command){
				if($cmd->testPermissionSilent($sender)){
					$message = TextFormat::LIGHT_PURPLE . "--------- " . TextFormat::WHITE . " Help: /" . $cmd->getName() . TextFormat::LIGHT_PURPLE . " ---------\n";
					$message .= TextFormat::WHITE . "Desc: " . TextFormat::WHITE . $cmd->getDescription() . "\n";
					$message .= TextFormat::WHITE . "Uso: " . TextFormat::WHITE . implode("\n" . TextFormat::WHITE, explode("\n", $cmd->getUsage())) . "\n";
